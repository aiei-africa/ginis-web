...
export default async function ElectoralPage() {
  ...
  const nationalValidVotes = nationalRows[0]?.valid_votes || 0;
  const nationalNDCVotes = nationalRows.find((r) => r.party_abbr === "NDC")?.votes || 0;
  const nationalNPPVotes = nationalRows.find((r) => r.party_abbr === "NPP")?.votes || 0;

  return (
    <div style={{ paddingBottom: 60 }}>
      <div style={{ padding: "16px 14px 12px" }}>
        <div style={{ fontSize: 9, letterSpacing: ".22em", color: T.gold, fontFamily: "var(--font-body)", fontWeight: 600, textTransform: "uppercase", marginBottom: 8 }}>
          GINIS · Pillar II · EC Gazetted Results
        </div>
        <h2 style={{ fontSize: 20, fontWeight: 700, fontFamily: "var(--font-display)", marginBottom: 3 }}>Electoral Intelligence</h2>
      ...
      <div style={{ margin: "0 12px 12px", border: `1px solid ${T.border}`, borderRadius: 4, padding: 14 }}>
        <div style={{ fontSize: 8.5, color: T.muted, letterSpacing: ".18em", textTransform: "uppercase", marginBottom: 10, fontFamily: "var(--font-body)", fontWeight: 600 }}>
          2024 NATIONAL RESULT
        </div>
        {[
          { label: "NDC · Mahama", v: nationalNDCVotes, c: "#1B6B3A" },
          { label: "NPP · Bawumia", v: nationalNPPVotes, c: "#003082" },
        ].map(({ label, v, c }) => {
          const p = nationalValidVotes ? ((v / nationalValidVotes) * 100).toFixed(2) : "0";
          return (
            <div key={label} style={{ marginBottom: 8 }}>
              ...
              <div style={{ height: 7, background: "rgba(240,165,0,0.08)", borderRadius: 2, overflow: "hidden" }}>
      ...
      <div style={{ padding: "0 12px" }}>
        <div style={{ fontSize: 8.5, color: T.muted, letterSpacing: ".18em", textTransform: "uppercase", marginBottom: 8, fontFamily: "var(--font-body)" }}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
          {regions.map((r) => {
            ...
            const validVotesRegion = r.valid_votes || 0;
            const topPct = validVotesRegion ? ((top.votes / validVotesRegion) * 100).toFixed(1) : "0";
            return (
              <Link
                key={r.region_id}
                href={`/electoral/${slugify(r.region_name)}`}
                style={{ background: "rgba(26,35,56,.5)", border: `1px solid ${T.border}`, borderRadius: 4, padding: "12px 11px", textAlign: "left", width: "100%", position: "relative", overflow: "hidden", display: "block" }}
              >
                ...
                <div style={{ fontSize: 14, fontWeight: 700, color: T.ivory, lineHeight: 1.1, marginBottom: 2, fontFamily: "var(--font-display)" }}>{r.region_name}</div>
                {top && (
                  <>
                    <div style={{ height: 4, borderRadius: 2, background: "rgba(240,165,0,0.08)", overflow: "hidden", display: "flex", margin: "6px 0" }}>
