...
export async function fetchRegionalResults(year = 2024, type = "PRESIDENTIAL"): Promise<RegionalRow[]> {
  const res = await fetch(`${API_URL}/electoral/regional?year=${year}&type=${type}`, { next: { revalidate: 3600 } });
  if (!res.ok) throw new Error("Failed to load regional results");
  return res.json();
}

export async function fetchConstituenciesByRegion(regionName: string, year = 2024, type = "PRESIDENTIAL"): Promise<ConstituencyRow[]> {
  const res = await fetch(`${API_URL}/electoral/constituencies?year=${year}&type=${type}&region=${encodeURIComponent(regionName)}`, { next: { revalidate: 3600 } });
...
export async function fetchConstituencyDetail(id: string, year = 2024, type = "PRESIDENTIAL"): Promise<ConstituencyRow[] | null> {
  const res = await fetch(`${API_URL}/electoral/constituencies/${id}?year=${year}&type=${type}`, { next: { revalidate: 3600 } });
...
export function groupRegional(rows: RegionalRow[]): RegionSummary[] {
  ...
  return Array.from(map.entries()).map(([region_name, group]) => {
